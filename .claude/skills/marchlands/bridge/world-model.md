...
**b. → Turning Point / plot points.** On an Altered/Interrupt Scene (AC always-on), when `adventure_crafter.py turning-point --campaign <dir>` invokes a Thread, **prefer a due clock or front** as that Thread. Read the threat's nature for the plot-point themes: Restless Dead → Tension/Mystery · Marauder Force → Action/Tension · Ambitious Order → Social/Mystery · Cursed Place → Mystery/Tension · Creeping Affliction → Tension/Personal. A **filled** clock → engineer its **Conclusion** plot point and remove the Thread.
